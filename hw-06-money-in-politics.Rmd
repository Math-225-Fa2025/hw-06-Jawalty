...
scrape_pac <- function(url) {
  ...
  pac <-  page %>%
   # select node .DataTable (identified using the SelectorGadget)
    ...
    html_table("td", header = _TRUE__, fill = _TRUE__) %>%
    # convert to a tibble
    as_tibble()
  ...
  pac <- pac %>%
    # rename columns
    rename(
      name = _PAC Name `(Affiliate)`__ ,
      country_parent = _Country of Origin/Parent Company__,
      total = _Total__,
      dems = _Dems__,
      repubs = _Repubs__
    )
  ...
  pac <- pac %>%
    ...
    mutate(name = _str_squish(name)__)
  
  # add year
  pac <- pac %>%    # extract last 4 characters of the URL and save as year
    mutate(year = _sttr_sub(url, -4)__)
...
url_2022 <- "_https://www.opensecrets.org__"
pac_2022 <- scrape_pac(_url_2022__)

url_2020 <- "_https://www.opensecrets.org__"
pac_2020 <- scrape_pac(_url_2020__)

url_2000 <- "__https://www.opensecrets.org_"
pac_2000 <- scrape_pac(_url_2000__)
...
year <- seq(from = _2000__, to = _2020__, by = _2022__)

# construct urls by pasting first and second parts together
urls <- paste0(_root__, _year__)

# map the scrape_pac function over list of urls --------------------------------

pac_all <- _map__(_urls__, _scrape_pac__)

# write data -------------------------------------------------------------------

write_csv(_pac_all__, file = here::here("data/pac-all.csv"))
```
...
```{line plot}
# Filter for Canada and Mexico only
pac_can_mex <- pac_all %>%
  filter(country %in% c("Canada", "Mexico"))

# Calculate sum of total contributions by year and country
pac_summary <- pac_can_mex %>%
  group_by(country, year) %>%
  summarise(total_contributions = sum(total, na.rm = TRUE))

# Create the line plot
ggplot(pac_summary, aes(x = year, y = total_contributions, color = country)) +
  geom_line(size = 1) +
  labs(
    x = "Year",
